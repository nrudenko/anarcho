...
from anarcho.web.app import create_app
...
@manager.command
def init_db():
    from anarcho.core import core_catalog
    for name, service in core_catalog.Services.all_providers():
        service()
    core_catalog.App.db.instance.metadata.create_all()


@manager.command
def gen_cert(host, name=None):
    """
    Generate ssl certificates for anarcho
    """
    if not name:
        name = 'anarcho_server'
...
@manager.command
def start(t=False, nodaemon=False):
    """
    Start anarcho daemon
    """
    if t or nodaemon:
        anarcho_cherry.run(manager.app)
    else:
        _start_d()
